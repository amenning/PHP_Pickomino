...
$dbUser='root';
$dbPassword='';
$dbName='pickominotest';
$dbUserTable='registrationtable';
?>
<div style="text-align: center; margin: auto; padding: 10px;">
	<form action="pickominoReg.php" method="POST">
...
if(isset($_POST['firstname']) && isset($_POST['lastname']) && isset($_POST['username']) && isset($_POST['password']) && isset($_POST['password_check']) && isset($_POST['email'])){
	if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['username'] && !empty($_POST['password'] && !empty($_POST['password_check']) && !empty($_POST['email'])))){
		...
		if($password!==$password_check){
			...
			if(!@mysql_connect($dbHost,$dbUser,$dbPassword) || !@mysql_select_db($dbName)){
				echo 'Error connecting to server';
			}else{
				$query_username_check="SELECT `id` FROM `".$dbUserTable."` WHERE `username`='".mysql_real_escape_string($username)."'";
				...
				if(@$query_username_check_run=mysql_query($query_username_check)){
					...
					if($query_username_check_num_rows!==0){
						...
						$query_register_user="INSERT INTO `".$dbUserTable."` VALUES('','".mysql_real_escape_string($username)."','".mysql_real_escape_string($passwordhash)."','".mysql_real_escape_string($firstname)."','".mysql_real_escape_string($lastname)."','".mysql_real_escape_string($email)."')";
